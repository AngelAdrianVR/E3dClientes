<template>
    <main @click="$inertia.get(route('catalog-product-company.show', catalogProductCompany.id))"
        class="border border-[#D9D9D9] dark:border-[#6B6767] rounded-2xl h-72 p-2 transition-all ease-linear duration-200 hover:shadow-md shadow-gray-600/50">
        <figure
            class="bg-[#F2F2F2] dark:bg-[#202020] p-2 rounded-2xl h-44 flex justify-center items-center relative group">
        ...
        <section class="py-2 flex flex-col justify-between h-24 mx-5">
            <h1 class="font-bold text-center text-sm truncate w-full px-4">{{ catalogProductCompany.catalog_product.name
                }}</h1>

            <h2 class="text-right">
                <span>P.U.</span>
                <strong class="mx-3">${{ catalogProductCompany.new_price?.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g,
                    ",") }}</strong>
