    ...
    <AppLayout :title="catalog_product_company?.catalog_product.name">
        <main class="p-2 md:p-10">
            ...
            <body class="my-9 lg:w-[85%] mx-auto md:grid grid-cols-2 gap-7">
                <figure class="rounded-2xl bg-[#F2F2F2] dark:bg-[#202020] p-4 h-72 flex justify-center items-center relative group">
                    ...
                    <!-- Ícono de lupa -->
                    <div v-if="catalog_product_company.catalog_product.media?.length"
                ...
                <section class="my-4">
                    <h1 class="font-bold md:text-lg text-center bg-gradient-to-r from-secondary to-error bg-clip-text text-transparent w-fit">{{ catalog_product_company?.catalog_product.name }}</h1>
                    <p class="my-5 md:text-lg">
                        <strong class="mr-2 font-bold">${{ catalog_product_company.new_price?.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</strong>
                        <span class="text-sm">{{ catalog_product_company.new_currency === '$MXN' ? 'MX' : 'USD' }}</span>
                        <span class="text-xs ml-5">(Precio unitario)</span>
                    </p>
                    <div class="grid grid-cols-3 gap-1 text-sm text-[#575757] dark:text-[#929292]">
                        <span>Número de parte</span>
                        <span class="col-span-2">{{ catalog_product_company.catalog_product.part_number }}</span>
                        <span>Descripción</span>
                        <span class="col-span-2">{{ catalog_product_company.catalog_product.description }}</span>
                        <span>Unidades de medida</span>
                        <span class="col-span-2">{{ catalog_product_company.catalog_product.measure_unit }}</span>
                        <h2 v-if="catalog_product_company.catalog_product.large || catalog_product_company.catalog_product.height || catalog_product_company.catalog_product.width" class="my-2 font-bold col-span-full text-lg bg-gradient-to-r from-secondary to-error bg-clip-text text-transparent w-fit">Dimensiones</h2>
                        <span v-if="catalog_product_company.catalog_product.large">Largo</span>
                        <span v-if="catalog_product_company.catalog_product.large" class="col-span-2">{{ catalog_product_company.catalog_product.large }}</span>
                        <span v-if="catalog_product_company.catalog_product.height">Alto</span>
                        <span v-if="catalog_product_company.catalog_product.height" class="col-span-2">{{ catalog_product_company.catalog_product.height }}</span>
                        <span v-if="catalog_product_company.catalog_product.width">Ancho</span>
                        <span v-if="catalog_product_company.catalog_product.width" class="col-span-2">{{ catalog_product_company.catalog_product.width }}</span>
                    </div>
                </section>
            </body>
        </main>
...
props:{
    catalog_product_company: Object
},
methods:{
